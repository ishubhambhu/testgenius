...
export function App() {
  ...
  useEffect(() => {
    const initAuth = async () => {
      try {
        ...
        // Show welcome screen only if no user is signed in
        if (!currentUser) {
  ...
  const loadTestHistory = useCallback(async () => {
    ...
    try {
      const history = await HistoryService.getTestHistory();
  ...
  // Load local storage data for non-authenticated users
  useEffect(() => {
    if (!user) {
  ...
  const handleStartTestAfterConfirmation = useCallback((updatedTestName: string) => {
    ...
    if (!currentTestSessionId || (questions.some(q => q.wasCorrectedByUser) && testPhase !== TestPhase.REVIEW) || isRetakeMode) { 
        setCurrentTestSessionId(crypto.randomUUID());
    }
  ...
  const saveOrUpdateTestInHistory = useCallback(async (questionsToScore: Question[], isFromReviewCorrection: boolean) => {
    ...
    if (user) {
      ...
      try {
        await HistoryService.saveTestHistory(historyEntry);
        await loadTestHistory(); // Reload from database
      } catch (error) {
  ...
  }, [currentSetupConfig, currentTestName, currentTestSessionId, user, loadTestHistory]);
  ...
  const handleDeleteHistoryEntry = async (idToDelete: string) => {
    if (window.confirm("Are you sure you want to delete this test from your history? This action cannot be undone.")) {
      if (user) {
        try {
          await HistoryService.deleteTestHistory(idToDelete);
          await loadTestHistory();
        } catch (error) {
          console.error('Error deleting history entry from database:', error);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      {showWelcomeScreen ? (
        renderContent() // Only render WelcomeScreen and nothing else
      ) : (
        <>
